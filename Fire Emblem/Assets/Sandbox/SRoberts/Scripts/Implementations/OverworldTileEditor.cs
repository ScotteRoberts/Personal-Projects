...
{
    [CustomEditor(typeof(OverworldTile), true)]
    [CanEditMultipleObjects]
    internal class OverworldTileEditor : Editor
    {
        ...
        public static Texture2D[] autoTransforms
        {
            get
            {
                if (s_AutoTransforms == null)
                {
                    ...
                    s_AutoTransforms[0] = OverworldTile.Base64ToTexture(s_Rotated);
                    s_AutoTransforms[1] = OverworldTile.Base64ToTexture(s_MirrorX);
                    s_AutoTransforms[2] = OverworldTile.Base64ToTexture(s_MirrorY);
                }
        ...
        public OverworldTile tile { get { return (target as OverworldTile); } }
        ...
        public void OnEnable()
        {
            if (tile.m_TilingRules == null)
                tile.m_TilingRules = new List<OverworldTile.TilingRule>();

            m_ReorderableList = new ReorderableList(tile.m_TilingRules, typeof(OverworldTile.TilingRule), true, true, true, true)
            {
        ...
        {
            OverworldTile.TilingRule rule = tile.m_TilingRules[index];
        ...
        {
            OverworldTile.TilingRule rule = new OverworldTile.TilingRule();
            rule.m_Output = OverworldTile.TilingRule.OutputSprite.Single;
        ...
        public override void OnInspectorGUI()
        {
            ...
            // Adding GUI entries for terrain stats.
            tile.m_Walkable = EditorGUILayout.Toggle("Walkable", tile.m_Walkable);

            var baseFields = typeof(OverworldTile).GetFields().Select(field => field.Name);
        ...
        internal static void RuleMatrixOnGUI(OverworldTile tile, Rect rect, OverworldTile.TilingRule tilingRule)
        {
            ...
            for (int y = 0; y <= 2; y++)
            {
                for (int x = 0; x <= 2; x++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            case OverworldTile.TilingRule.Transform.Rotated:
                                GUI.DrawTexture(r, autoTransforms[0]);
                                break;
                            case OverworldTile.TilingRule.Transform.MirrorX:
                                GUI.DrawTexture(r, autoTransforms[1]);
                                break;
                            case OverworldTile.TilingRule.Transform.MirrorY:
                        ...
                        if (Event.current.type == EventType.MouseDown && r.Contains(Event.current.mousePosition))
                        {
                            tilingRule.m_RuleTransform = (OverworldTile.TilingRule.Transform)(((int)tilingRule.m_RuleTransform + 1) % 4);
                            GUI.changed = true;
                            Event.current.Use();
                        }
                    }
        ...
        {
            public OverworldTile.TilingRule m_Rule;
            public OverworldTile.TilingRule.Transform m_NewValue;

            public MenuItemData(OverworldTile.TilingRule mRule, OverworldTile.TilingRule.Transform mNewValue)
            {
                this.m_Rule = mRule;
        ...
        internal static void SpriteOnGUI(Rect rect, OverworldTile.TilingRule tilingRule)
        {
        ...
        internal static void RuleInspectorOnGUI(Rect rect, OverworldTile.TilingRule tilingRule)
        {
            float y = rect.yMin;
            EditorGUI.BeginChangeCheck();
            GUI.Label(new Rect(rect.xMin, y, k_LabelWidth, k_SingleLineHeight), "Rule");
            tilingRule.m_RuleTransform = (OverworldTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RuleTransform);
            ...
            tilingRule.m_Output = (OverworldTile.TilingRule.OutputSprite)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_Output);
            y += k_SingleLineHeight;

            if (tilingRule.m_Output == OverworldTile.TilingRule.OutputSprite.Animation)
            {
            ...
            if (tilingRule.m_Output == OverworldTile.TilingRule.OutputSprite.Random)
            {
                ...
                tilingRule.m_RandomTransform = (OverworldTile.TilingRule.Transform)EditorGUI.EnumPopup(new Rect(rect.xMin + k_LabelWidth, y, rect.width - k_LabelWidth, k_SingleLineHeight), tilingRule.m_RandomTransform);
                y += k_SingleLineHeight;
            }

            if (tilingRule.m_Output != OverworldTile.TilingRule.OutputSprite.Single)
            {
        ...
        [Serializable]
        class OverworldTileRuleWrapper
        {
            [SerializeField]
            public List<OverworldTile.TilingRule> rules = new List<OverworldTile.TilingRule>();
        }

        [MenuItem("CONTEXT/OverworldTile/Copy All Rules")]
        private static void CopyAllRules(MenuCommand item)
        {
            OverworldTile tile = item.context as OverworldTile;
            if (tile == null)
                return;

            OverworldTileRuleWrapper rulesWrapper = new OverworldTileRuleWrapper();
        ...
        [MenuItem("CONTEXT/OverworldTile/Paste Rules")]
