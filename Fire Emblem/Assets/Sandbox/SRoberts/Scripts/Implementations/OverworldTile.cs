...
// Figure out a namespace.
namespace UnityEngine
{
    public class OverworldTile<T> : OverworldTile
    {
        public sealed override Type m_NeighborType { get { return typeof(T); } }
    }
    [Serializable]
    [CreateAssetMenu(fileName = "New Overworld Tile", menuName = "Tiles/Overworld Tile")]
    public class OverworldTile : TileBase
    {
#if UNITY_EDITOR
        ...
        public virtual void RuleOnGUI(Rect rect, Vector2Int pos, int neighbor)
        {
            switch (neighbor)
            {
                case OverworldTile.TilingRule.Neighbor.DontCare:
                    break;
                case OverworldTile.TilingRule.Neighbor.This:
                    GUI.DrawTexture(rect, arrows[pos.y * 3 + pos.x]);
                    break;
                case OverworldTile.TilingRule.Neighbor.NotThis:
        ...
        // Adding public values for the terrain costs 
